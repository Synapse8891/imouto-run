...
function activate(context) {
    ...
    function isSupportedDocument(doc) {
        ...
        return (lower.endsWith('.py') || langId === 'python' ||
    ...
    const DEFAULT_LANGUAGE_COMMANDS = {
        py: 'python "{path}"',
    ...
    async function runLanguageShortcut(lang, target) {
        ...
        output.clear();
        const cfg = vscode.workspace.getConfiguration('imoutoRun');
        const verbosity = cfg.get('verbosity', 'simple');
        if (verbosity === 'verbose') {
            output.appendLine(`--- 実行コマンド: ${command}`);
        }
        (0, child_process_1.exec)(command, { maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => {
    ...
    function displayError(rawErr, stdout, imoutoSpeech) {
        const cfg = vscode.workspace.getConfiguration('imoutoRun');
    ...
    // エラーメッセージを簡潔に要約する関数
    function summarizeError(rawErr) {
        // 最初の行と最後のエラー行を抽出
    ...
    let openListenerDisposable;
    let saveListenerDisposable;
    // 設定に応じてリスナーを登録/解除する関数
    function registerListenersFromConfig() {
        const cfg = vscode.workspace.getConfiguration('imoutoRun');
        const autoStartup = cfg.get('autoRunOnStartup', true);
        const autoOpen = cfg.get('autoRunOnOpen', false);
        const autoSave = cfg.get('autoRunOnSave', false);
        // 起動時の実行
        ...
        // ファイルが開かれたとき
        if (autoOpen) {
            openListenerDisposable = vscode.workspace.onDidOpenTextDocument((doc) => {
                if (isSupportedDocument(doc)) {
                    runDocument(doc);
                }
            });
...
// エラーメッセージを妹口調に変換する関数
function translateToImouto(rawErr) {
